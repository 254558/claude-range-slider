<template>
  ...
  >
    <defs>
      ...
      <clipPath id="squircleTrackPath" clipPathUnits="objectBoundingBox">
        <path
          d="M 0.033,0
...
function initWebGL() {
  ...
  function bootstrap() {
    ...
    // ========== 修改点 ==========
    const dpr = window.devicePixelRatio                    // 移除封顶
    canvas.width = Math.round(rect.width * dpr)            // 四舍五入
    canvas.height = Math.round(rect.height * dpr)          // 四舍五入
    // ==========================
...
// 改为：
float fade_edge = 0.6
    + sin(uv.y * 12.0 + t * 0.8) * 0.04
    + sin(uv.y * 7.0  - t * 0.5) * 0.03;
col *= smoothstep(0.1, fade_edge, uv.x);
    ...
    const simProg = makeProg(VS, FS_SIM)
    const blurProg = makeProg(VS, FS_BLUR)
    const compProg = makeProg(VS, FS_COMP)
    ...
    const U = {
      simTime: gl.getUniformLocation(simProg, 'u_time'),
      simSlider: gl.getUniformLocation(simProg, 'u_slider'),
      simElapsed: gl.getUniformLocation(simProg, 'u_elapsed'),
      simBack: gl.getUniformLocation(simProg, 'u_back'),
    ...
    function render(t) {
      ...
      if (!currentActive && !wasActive) return
...
.track-wrapper {
  ...
  background: #0c0c0c;
  isolation: isolate;
  clip-path: url(#squircleTrackPath);
}
